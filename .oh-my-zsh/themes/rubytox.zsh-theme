# RUBYTOX ZSH Theme forked from AVIT

# settings
typeset +H _current_dir="%{$fg_bold[blue]%}%3~%{$reset_color%} "
typeset +H _return_status="%{$fg_bold[red]%}%(?..✗ )%{$reset_color%}"
typeset +H _hist_no="%{$fg[grey]%}%h%{$reset_color%}"
typeset +h _user_name="%{$fg_bold[green]%}[ %{$fg_bold[cyan]%}$USER %{$fg_bold[green]%}]%{$reset_color%}"

# PROMPT='
# ${_user_name} ${_current_dir} $(git_prompt_info) $(ruby_prompt_info)
# %{%(!.%F{red}.%F{white})%}%{$fg_bold[green]%}▶%{$reset_color%} '
PROMPT='
${_return_status}${_user_name} ${_current_dir} $(git_prompt_info) $(ruby_prompt_info)
%{%(!.%F{red}.%F{white})%}%{$fg_bold[green]%}❱❱❱%{$reset_color%} '
